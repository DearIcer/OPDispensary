...
namespace OPDispensary.Application;
...
public class RoleService : IRoleService
{
    ...
    public async Task<List<ResTreeSelector>> ResourceTreeSelector()
    {
        ...
        //获取角色资源关系
        var relations =
            await _relationService.GetRelationListByObjectIdListAndCategory(user.RoleIdList,
        ...
        var parentMenuList =
            menuList.Where(it => ownMenuList.Select(m => m.ParentId).Contains(it.Id)).ToList(); //获取所有父级菜单 
        ...
        List<SysResource> ownButtonList)
    {
        ...
        var parentMenuList = ownMenuList.Where(it => it.ParentId == OPDispensaryConst.ZERO).ToList(); //获取一级目录
        //遍历一级目录
                    ...
                    foreach (var menu in menuList)
                        ...
                        if (menu.MenuType is SysResourceConst.MENU or SysResourceConst.SUBSET)
                        {
                            ...
                            roleGrantResourceMenus.Add(new ResTreeSelector.RoleGrantResourceMenu
                            {
                                ...
                                Module = moduleId,
                                Title = _resourceService.GetRoleGrantResourceMenuTitle(menuList,
