...
public class OrgService : DbRepository<SysOrg>, IOrgService
{
    ...
    public async Task Copy(SysOrgCopyInput input)
    {
        ...
        {
            if (dataScope is { Count: > 0 } &&
                (!dataScope.ContainsAll(input.Ids) || !dataScope.Contains(input.TargetId))) //判断目标机构和需要复制的机构是否都在数据范围里面
    ...
    private async Task CheckInput(SysOrg sysOrg, string operate)
    {
        ...
        if (sysOrg.ParentId == OPDispensaryConst.ZERO) throw Oops.Bah($"不能{operate}根机构");
