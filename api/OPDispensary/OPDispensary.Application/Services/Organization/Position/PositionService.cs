...
public class PositionService : DbRepository<SysPosition>, IPositionService
{
    ...
    private async Task CheckInput(SysPosition sysPosition, string operate)
    {
        ...
        await _sysUserService.CheckApiDataScope(sysPosition.OrgId, sysPosition.CreateUserId.GetValueOrDefault(),
            errorMessage);
    }
    ...
    public async Task<SysPosition> Detail(BaseIdInput input)
    {
        ...
        //判断数据范围
        await _sysUserService.CheckApiDataScope(position.OrgId, position.CreateUserId.GetValueOrDefault(),
            "您没有权限查看该机构");
        return position;
    }
