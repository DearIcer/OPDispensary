...
public class SM4CryptoUtil
{
    ...
    {
        var n = ((long)(b[i] & 0xff) << 24) | (uint)((b[i + 1] & 0xff) << 16) | (uint)((b[i + 2] & 0xff) << 8) |
    ...
    private static byte[] Padding(byte[] input, int mode)
    {
        ...
        if (mode == 1)
        {
            ...
            Array.Copy(input, 0, ret, 0,
                input.Length);
            for (var i = 0; i < p; i++) ret[input.Length + i] = (byte)p;
        }
        else
        {
    ...
    {
        if (ctx.IsPadding && ctx.Mode == 1) input = Padding(input, 1);
        ...
        if (ctx.IsPadding && ctx.Mode == 0) bous = Padding(bous, 0);
        return bous;
    }
        ...
        if (ctx.Mode == 1)
        {
            for (var j = 0; length > 0; length -= 16, j++)
            {
                ...
                Array.Copy(bins, j * 16, inBytes, 0,
                    length > 16 ? 16 : length);
                for (i = 0; i < 16; i++) outBytes[i] = (byte)(inBytes[i] ^ iv[i]);
                Sm4OneRound(ctx.Key, outBytes, out1);
                Array.Copy(out1, 0, iv, 0,
                    16);
                for (var k = 0; k < 16; k++) bousList.Add(out1[k]);
            }
        }
        else
        {
            ...
            for (var j = 0; length > 0; length -= 16, j++)
            {
                ...
                Array.Copy(inBytes, 0, temp, 0,
                    16);
                Sm4OneRound(ctx.Key, inBytes, outBytes);
                for (i = 0; i < 16; i++) out1[i] = (byte)(outBytes[i] ^ iv[i]);
