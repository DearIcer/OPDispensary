...
namespace OPDispensary.Plugin.Aop;
    ...
    private dynamic CheckCache(MethodInfo method, object[] args)
    {
        ...
        if (cacheAttribute != null)
        {
            ...
            var cacheKey = cacheAttribute.CustomKeyValue
                           ?? CustomCacheKey(cacheAttribute.KeyPrefix, method,
                               args); //如果redisKey值，如果有自定义值就用自定义Key，否则以前缀+系统自动生成的Key
            string cacheValue;
            if (cacheAttribute.StoreType == CacheConst.CACHE_HASH) //如果存的是Hash值
                cacheValue =
                    redisManager.HashGet<string>(cacheKey, args[0].ToString())[0]; //从redis获取Hash数据取第一个,注意是 string 类型
            else
...
internal static class GetCacheKey
{
    ...
    public static string GetArgumentValue(object arg)
    {
        ...
        if (arg != null)
        {
            ...
            if (arg.GetType().IsClass) return MD5Encryption.Encrypt(JsonConvert.SerializeObject(arg));
        }
    ...
    private static string Resolve(Expression expression)
    {
        ...
        if (expression is BinaryExpression)
        {
            ...
            if (binary.Left is MethodCallExpression &&
                binary.Right is ConstantExpression) //解析x=>x.Name.Contains("xxx")==false这类的
            {
    ...
    private static string ResolveLinqToObject(Expression expression, object value,
