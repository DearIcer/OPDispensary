...
public class AuthEventSubscriber : IEventSubscriber, ISingleton
{
    ...
    public async Task Login(EventHandlerExecutingContext context)
    {
        ...
        //获取用户token列表
        var tokenInfos =
            _simpleCacheService.HashGetOne<List<TokenInfo>>(CacheConst.CACHE_USER_TOKEN, sysUser.Id.ToString());
        ...
        if (userToken != null)
        {
            ...
            {
                var pwdUpdateDefault = loginPolicy.First(x => x.ConfigKey == SysConfigConst.PWD_UPDATE_DEFAULT)
                    .ConfigValue.ToBoolean(); //获取初始化提醒
                //如果密码初始化提醒为true
                if (pwdUpdateDefault)
            ...
            {
                var pwdRemind = loginPolicy.First(x => x.ConfigKey == SysConfigConst.PWD_REMIND).ConfigValue
                    .ToBoolean(); //获取密码提醒天数
                if (pwdRemind)
                {
                    var pwdRemindDay = loginPolicy.First(x => x.ConfigKey == SysConfigConst.PWD_REMIND_DAY).ConfigValue
                        .ToInt(); //获取密码提醒时间
