...
public class NoticeEventSubscriber : IEventSubscriber, ISingleton
{
    ...
    public async Task UserLoginOut(EventHandlerExecutingContext context)
    {
        ...
        //遍历token列表获取客户端ID列表
        loginEvent.TokenInfos.ForEach(it => { clientIds.AddRange(it.ClientIds); });
    }
    ...
    public async Task NewMessage(EventHandlerExecutingContext context)
    {
        ...
        //获取用户token列表
        var tokenInfos =
            _simpleCacheService.HashGet<List<TokenInfo>>(CacheConst.CACHE_USER_TOKEN,
                newMessageEvent.UserIds.ToArray());
