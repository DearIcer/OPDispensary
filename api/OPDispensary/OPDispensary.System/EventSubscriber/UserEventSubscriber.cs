...
public class UserEventSubscriber : IEventSubscriber, ISingleton
{
    ...
    public async Task DeleteUserCacheByRoleIds(EventHandlerExecutingContext context)
    {
        ...
        //获取用户和角色关系
        var relations = await relationService.GetRelationListByTargetIdListAndCategory(
            roleIds.Select(it => it.ToString()).ToList(),
    ...
    public async Task DeleteUserTokenByOrgIds(EventHandlerExecutingContext context)
    {
        ...
        using var scope = _services.CreateScope();
        // 获取用户ID列表
        var userIds = await DbContext.DB.QueryableWithAttr<SysUser>().Where(it => orgIds.Contains(it.OrgId))
            .Select(it => it.Id).ToListAsync();
        //从redis中删除所属机构的用户token
        _simpleCacheService.HashDel<List<Token>>(CacheConst.CACHE_USER_TOKEN,
            userIds.Select(it => it.ToString()).ToArray());
    }
    ...
    public async Task DeleteUserTokenByModuleId(EventHandlerExecutingContext context)
    {
        ...
        var roleModuleRelations =
            await relationService.GetRelationListByTargetIdAndCategory(moduleId.ToString(),
                CateGoryConst.RELATION_SYS_ROLE_HAS_MODULE); //角色模块关系
        var userModuleRelations =
            await relationService.GetRelationListByTargetIdAndCategory(moduleId.ToString(),
                CateGoryConst.RELATION_SYS_USER_HAS_MODULE); //用户模块关系
        ...
        var userRoleRelations = await relationService.GetRelationListByTargetIdListAndCategory(
            roleIds.Select(it => it.ToString()).ToList(),
