...
public class UserManager
{
    ...
    ///     当前用户Id
    /// </summary>
    public static long UserId => (App.User?.FindFirst(ClaimConst.USER_ID)?.Value).ToLong();
    ...
    ///     是否超级管理员
    /// </summary>
    public static bool SuperAdmin => (App.User?.FindFirst(ClaimConst.IS_SUPER_ADMIN)?.Value).ToBoolean();

    /// <summary>
    ///     机构ID
    /// </summary>
    public static long OrgId => (App.User?.FindFirst(ClaimConst.ORG_ID)?.Value).ToLong();

    public static long? TenantId =>
        //如果有租户ID则返回租户ID，否则返回null
        (App.User?.FindFirst(ClaimConst.TENANT_ID)?.Value).ToLong();
}