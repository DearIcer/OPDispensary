...
public class MinioUtils : ITransient
{
    ...
    private async void InitClient()
    {
        ...
        var accessKey =
            configs.Where(it => it.ConfigKey == SysConfigConst.FILE_MINIO_ACCESS_KEY)
                .FirstOrDefault(); //MINIO文件AccessKey
        var secretKey =
            configs.Where(it => it.ConfigKey == SysConfigConst.FILE_MINIO_SECRET_KEY)
                .FirstOrDefault(); //MINIO文件SecretKey
        var endPoint =
            configs.Where(it => it.ConfigKey == SysConfigConst.FILE_MINIO_END_POINT).FirstOrDefault(); //MINIO文件EndPoint
        var bucketName = configs.Where(it => it.ConfigKey == SysConfigConst.FILE_MINIO_DEFAULT_BUCKET_NAME)
            .FirstOrDefault(); //MINIO文件默认存储桶
        ...
        try
        {
            ...
            MinioClient = new MinioClient().WithEndpoint(_defaultEndPoint)
                .WithCredentials(accessKey.ConfigValue, secretKey.ConfigValue)
                .Build(); //初始化minio对象
    ...
    /// <returns></returns>
    public async Task<string> PutObjectAsync(string objectName, IFormFile file,
        string contentType = "application/octet-stream")
    {
        try
        {
            using var fileStream = file.OpenReadStream(); //获取文件流
            var putObjectArgs = new PutObjectArgs().WithBucket(_defaultBucketName).WithObject(objectName)
    ...
    public async Task<MemoryStream> DownloadFileAsync(string objectName)
    {
        ...
        try
        {
            var getObjectArgs = new GetObjectArgs().WithBucket(_defaultBucketName).WithObject(objectName)
                .WithCallbackStream(cb => { cb.CopyTo(stream); });
