...
public class UserCenterService : DbRepository<SysUser>, IUserCenterService
{
    ...
    public UserCenterService(ISysUserService userService, IRelationService relationService,
    ...
    public async Task<List<SysResource>> GetLoginMenu(BaseIdInput input)
    {
        ...
        if (userInfo != null)
        {
            ...
            //获取我的菜单列表,只显示启用的
            var myMenus = allMenuList.Where(it => menuIdList.Contains(it.Id) && it.Status == CommonStatusConst.ENABLE)
                .ToList();
            // 对获取到的角色对应的菜单列表进行处理，获取父列表
            var parentList = GetMyParentMenus(allMenuList, myMenus);
            myMenus.AddRange(parentList); //合并列表
            // 遍历单页列表
            allSpaList.ForEach(it => { it.ParentId = OPDispensaryConst.ZERO; });
    ...
    public async Task<string> GetLoginWorkbench()
    {
        ...
        //如果没数据去系统配置里取默认的工作台
        var devConfig =
            await _configService.GetByConfigKey(CateGoryConst.CONFIG_SYS_BASE,
                SysConfigConst.SYS_DEFAULT_WORKBENCH_DATA);
        if (devConfig != null) return devConfig.ConfigValue.ToLower(); //返回工作台信息
        return "";
    }
    ...
    public async Task<List<LoginOrgTreeOutput>> LoginOrgTree()
    {
        ...
        var topOrg = parentOrgList.Where(it => it.ParentId == OPDispensaryConst.ZERO).FirstOrDefault(); //获取顶级节点
        if (topOrg != null)
        {
    ...
    public async Task<List<SysResource>> ShortcutTree()
    {
        ...
        else
        {
            ...
            var roleResourceList =
                (await _relationService.GetRelationListByObjectIdListAndCategory(roleIds,
                    CateGoryConst.RELATION_SYS_ROLE_HAS_RESOURCE))
    ...
    {
        //关系表保存个人工作台
        await _relationService.SaveRelation(CateGoryConst.RELATION_SYS_USER_WORKBENCH_DATA, UserManager.UserId, null,
            input.WorkbenchData,
            true);
    }
    ...
    public async Task UpdatePassword(UpdatePasswordInput input)
    {
        ...
        var containNumber = loginPolicy.First(it => it.ConfigKey == SysConfigConst.PWD_CONTAIN_NUM).ConfigValue
            .ToBoolean(); //是否包含数字
        var containLower = loginPolicy.First(it => it.ConfigKey == SysConfigConst.PWD_CONTAIN_LOWER).ConfigValue
            .ToBoolean(); //是否包含小写
        var containUpper = loginPolicy.First(it => it.ConfigKey == SysConfigConst.PWD_CONTAIN_UPPER).ConfigValue
            .ToBoolean(); //是否包含大写
        var containChar = loginPolicy.First(it => it.ConfigKey == SysConfigConst.PWD_CONTAIN_CHARACTER).ConfigValue
            .ToBoolean(); //是否包含特殊字符
        var minLength =
            loginPolicy.First(it => it.ConfigKey == SysConfigConst.PWD_MIN_LENGTH).ConfigValue.ToInt(); //最小长度
        ...
        await UpdateSetColumnsTrueAsync(it => new SysUser { Password = newPassword },
