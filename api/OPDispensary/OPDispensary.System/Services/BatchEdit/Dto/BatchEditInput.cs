    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        if (Status == CommonStatusConst.ENABLE)
        {
            ...
            if (DataType.Contains("api") && (string.IsNullOrEmpty(RequestUrl) || string.IsNullOrEmpty(RequestType)
                                                                              || string.IsNullOrEmpty(RequestLabel) ||
...
{
    [Required(ErrorMessage = "字段名必填")] public string TableColumn { get; set; }

    [Required(ErrorMessage = "字段值必填")] public object ColumnValue { get; set; }
}