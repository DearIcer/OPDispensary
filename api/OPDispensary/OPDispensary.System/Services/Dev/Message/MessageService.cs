...
public class MessageService : DbRepository<SysMessage>, IMessageService
{
    ...
    public async Task<SqlSugarPagedList<SysMessage>> MyMessagePage(MessagePageInput input, long userId)
    {
        var query = Context.Queryable<SysMessageUser>().LeftJoin<SysMessage>((u, m) => u.MessageId == m.Id)
            ...
            .OrderBy((u, m) => u.Read).OrderBy((u, m) => m.CreateTime, OrderByType.Desc).Select((u, m) => new SysMessage
            {
                Id = m.Id.SelectAll(),
                Read = u.Read
            });
    ...
    public async Task<MessageDetailOutPut> Detail(BaseIdInput input, bool isSelf = false)
    {
        ...
        if (message != null)
        {
            ...
            var myMessage = messageUsers.Where(it => it.UserId == UserManager.UserId && it.MessageId == input.Id)
            ...
            if (!isSelf) //如果不是自己则把所有的用户都列出来
            {
                ...
                var userInfos = await Context.Queryable<SysUser>().Where(it => userIds.Contains(it.Id))
    ...
    {
        var messageUserRep = ChangeRepository<DbRepository<SysMessageUser>>(); //切换仓储
        await Context.Deleteable<SysMessageUser>().Where(it => it.UserId == userId && it.MessageId == input.Id)
            .IsLogic().ExecuteCommandAsync(); //逻辑删除
    }
    ...
    public async Task<int> UnReadCount(long userId)
    {
        ...
        //获取未读数量
        var unRead =
            await messageUserRep.CountAsync(it => it.UserId == userId && it.Read == false && it.IsDelete == false);
        return unRead;
    }
}