...
public class VisitLogService : DbRepository<SysLogVisit>, IVisitLogService
{
    ...
    public async Task<SqlSugarPagedList<SysLogVisit>> Page(VisitLogPageInput input)
    {
        var query = Context.Queryable<SysLogVisit>()
            ...
            .SplitTable(tabs => tabs.Take(_maxTabs)).OrderByIF(!string.IsNullOrEmpty(input.SortField),
    ...
    {
        var data = await Context.Queryable<SysLogVisit>().SplitTable(tabs => tabs.Take(_maxTabs))
