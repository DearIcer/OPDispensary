...
using OPDispensary.Core.Extension;
...
public class SysUserService : DbRepository<SysUser>, ISysUserService
{
    ...
    public async Task<SysUser> GetUserByAccount(string account, long? tenantId = null)
    {
        ...
        if (userId != OPDispensaryConst.ZERO)
        {
    ...
    public async Task<SysUser> GetUserByPhone(string phone, long? tenantId = null)
    {
        ...
        if (userId > 0) return await GetUserById(userId); //获取用户信息
        return null;
    }
    ...
    public async Task<SysUser> GetUserById(long userId)
    {
        ...
        if (sysUser == null) sysUser = await GetUserFromDb(userId); //从数据库拿用户信息
        return sysUser;
    }
        ...
        var result = await Context.Queryable<SysUser>()
            ...
            .WhereIF(input.RoleId > 0,
                u => SqlFunc.Subqueryable<SysRelation>()
                    .Where(r => r.TargetId == input.RoleId.ToString() && r.ObjectId == u.Id &&
                                r.Category == CateGoryConst.RELATION_SYS_USER_HAS_ROLE)
                    .Any()) //指定角色
    ...
    public async Task<SysUser> Detail(BaseIdInput input)
    {
        ...
        if (user != null) user.Password = null; //清空密码
        return user;
    }
    ...
    public async Task GrantResource(UserGrantResourceInput input)
    {
        ...
        if (sysUser != null)
        {
            ...
            var result = await Tenant.UseTranAsync(async () =>
            {
                ...
                await relationRep.DeleteAsync(it => (it.ObjectId == sysUser.Id &&
                                                     (it.Category == CateGoryConst.RELATION_SYS_USER_HAS_PERMISSION
                                                      || it.Category ==
                                                      CateGoryConst.RELATION_SYS_USER_HAS_RESOURCE)) ||
                                                    it.Category == CateGoryConst.RELATION_SYS_USER_HAS_MODULE);
    ...
    public async Task GrantPermission(GrantPermissionInput input)
    {
        ...
        if (sysUser != null)
        {
            ...
            var extJsons = input.GrantInfoList.Select(it => it.ToJson()).ToList(); //拓展信息
            await _relationService.SaveRelationBatch(CateGoryConst.RELATION_SYS_USER_HAS_PERMISSION, input.Id, apiUrls,
                extJsons,
                true); //添加到数据库
    ...
    public async Task Delete(BaseIdListInput input)
    {
        ...
        if (ids.Count > 0)
        {
            var containsSuperAdmin =
                await IsAnyAsync(it => it.Account == SysRoleConst.SUPER_ADMIN && ids.Contains(it.Id)); //判断是否有超管
            ...
            if (result.IsSuccess) //如果成功了
            {
                ...
                await _relationService.RefreshCache(CateGoryConst
                    .RELATION_SYS_USER_HAS_MODULE); //关系表刷新RELATION_SYS_USER_HAS_MODULE缓存
                await _relationService.RefreshCache(CateGoryConst
                    .RELATION_SYS_USER_SCHEDULE_DATA); //关系表刷新RELATION_SYS_USER_SCHEDULE_DATA缓存
                await _relationService.RefreshCache(CateGoryConst
                    .RELATION_SYS_USER_WORKBENCH_DATA); //关系表刷新RELATION_SYS_USER_WORKBENCH_DATA缓存
    ...
    public void DeleteUserFromRedis(List<long> ids)
    {
        ...
        if (sysUsers.Count > 0)
        {
            ...
            {
                var tenantIds = sysUsers.Where(it => it.TenantId != null).Select(it => it.TenantId.Value).Distinct()
                    .ToArray(); //租户id列表
        ...
        foreach (var item in data)
        {
            ...
            if (!string.IsNullOrEmpty(item.PositionName))
            {
                ...
                else
                {
                    //根据部门ID和职位名判断是否有职位
                    var position =
                        sysPositions.FirstOrDefault(u => u.OrgId == item.OrgId && u.Name == item.PositionName);
    ...
    private async Task<string> GetDefaultPassWord(bool isSm4 = false)
    {
        //获取默认密码
        var defaultPassword =
            (await _configService.GetByConfigKey(CateGoryConst.CONFIG_PWD_POLICY, SysConfigConst.PWD_DEFAULT_PASSWORD))
            .ConfigValue;
    ...
    private async Task CheckInput(SysUser sysUser)
    {
        ...
        if (sysUser.DirectorId != null)
            if (sysUser.DirectorId.Value == UserManager.UserId)
                throw Oops.Bah("不能设置自己为主管");
    }
    ...
    private async Task<SysUser> GetUserFromDb(long userId)
    {
        ...
        if (sysUser != null)
        {
            ...
            var scopeOrgChildList =
                (await _sysOrgService.GetChildListById(sysUser.OrgId, true, sysOrgList)).Select(it => it.Id)
                .ToList(); //获取所属机构的下级机构Id列表
