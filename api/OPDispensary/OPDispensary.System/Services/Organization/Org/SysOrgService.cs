...
public class SysOrgService : DbRepository<SysOrg>, ISysOrgService
{
    ...
    public SysOrgService(ILogger<SysOrgService> logger, ISimpleCacheService simpleCacheService,
    ...
    public async Task Edit(SysOrgEditInput input, string name = SystemConst.SYS_ORG)
    {
        ...
        if (await UpdateAsync(sysOrg)) //更新数据
        {
            ...
            {
                var orgIds = await GetOrgChildIds(sysOrg.Id); //获取所有下级
    ...
    public async Task Delete(BaseIdListInput input, string name = SystemConst.SYS_ORG)
    {
        ...
        if (ids.Count > 0)
        {
            ...
            //判断组织下是否有角色
            var hasRole = await Context.Queryable<SysRole>().Where(it => sysDeleteOrgList.Contains(it.OrgId.Value))
                .CountAsync() > 0;
            if (hasRole) throw Oops.Bah($"请先删除{name}下的角色");
            // 判断组织下是否有职位
            var hasPosition = await Context.Queryable<SysPosition>().Where(it => sysDeleteOrgList.Contains(it.OrgId))
                .CountAsync() > 0;
    ...
    public async Task<List<SysOrg>> GetListAsync(bool showDisabled = true)
    {
        ...
        if (!showDisabled) sysOrgList = sysOrgList.Where(it => it.Status == CommonStatusConst.ENABLE).ToList();
        return sysOrgList;
    }
    ...
    public async Task<SqlSugarPagedList<SysOrg>> Page(SysOrgPageInput input)
    {
        var query = Context.Queryable<SysOrg>()
            .WhereIF(input.ParentId > 0,
                it => it.ParentId == input.ParentId || it.Id == input.ParentId ||
                      SqlFunc.JsonLike(it.ParentIdList, input.ParentId.ToString())) //父级
    ...
    {
        long parentId = OPDispensaryConst.ZERO; //父级ID
        //获取所有组织
    ...
    public async Task<SysOrg> Detail(BaseIdInput input)
    {
        ...
        if (orgDetail.DirectorId != null)
            //获取主管信息
            orgDetail.DirectorInfo = await Tenant.QueryableWithAttr<SysUser>()
                .Where(it => it.Id == orgDetail.DirectorId).Select(it =>
    ...
    public async Task Copy(SysOrgCopyInput input)
    {
        ...
        if (target != null)
        {
            ...
            {
                it.Names = it.Name;
                if (it.ParentId != OPDispensaryConst.ZERO)
                {
            ...
            var result = await Tenant.UseTranAsync(async () =>
            {
                ...
                if (addPositionList.Count > 0) await Context.Insertable(addPositionList).ExecuteCommandAsync();
            });
    ...
    /// <inheritdoc />
    public List<SysOrg> ConstructOrgTrees(List<SysOrg> orgList, long parentId = OPDispensaryConst.ZERO)
    {
        //找下级ID列表
    ...
    private async Task CheckInput(SysOrg sysOrg, string name)
    {
        ...
        if (sysOrgList.Any(it =>
                it.ParentId == sysOrg.ParentId && it.Name == sysOrg.Name && it.Id != sysOrg.Id)) //判断同级是否有名称重复的
    ...
    /// <returns></returns>
    public Tuple<List<SysOrg>, List<SysPosition>> CopySysOrgChildren(List<SysOrg> orgList, long parentId,
        long newParentId,
        bool isCopyPosition,
        ...
        out string names, out List<long> parentIdList)
    {
        ...
        foreach (var item in parents) names += $"{item.Name}/";
        names += orgName; //赋值全称
