...
public class ImportExportService : IImportExportService
{
    ...
    public void ImportVerification(IFormFile file, int maxSize = 30, string[] allowTypes = null)
    {
        ...
        if (!allowTypeS.Contains(fileSuffix)) throw Oops.Bah("文件格式错误");
    }

    /// <inheritdoc />
    public ImportPreviewOutput<T> TemplateDataVerification<T>(ImportResult<T> importResult)
        where T : ImportTemplateInput
    {
        ...
        importResult.ImporterHeaderInfos.ForEach(it =>
        {
            ...
            {
                Title = it.Header.Name.Split("(")[0], DataIndex = it.PropertyName.FirstCharToLower()
            }; //定义表头,部分表头有说明用(分组去掉说明
    ...
    /// <inheritdoc />
    public ImportResultOutPut<T> GetImportResultPreview<T>(List<T> data, out List<T> importData)
        where T : ImportTemplateInput
    {
        //定义结果
