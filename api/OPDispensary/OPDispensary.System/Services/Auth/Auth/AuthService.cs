...
namespace OPDispensary.System.Services.Auth;
...
public class AuthService : IAuthService
{
    ...
    public AuthService(ISimpleCacheService simpleCacheService, IEventPublisher eventPublisher,
    ...
    {
        var config =
            await _configService.GetByConfigKey(CateGoryConst.CONFIG_LOGIN_POLICY, SysConfigConst.LOGIN_CAPTCHA_TYPE);
    ...
    public async Task CheckCaptcha(LoginInput input)
    {
        //判断是否有验证码
        var sysBase =
            await _configService.GetByConfigKey(CateGoryConst.CONFIG_LOGIN_POLICY, SysConfigConst.LOGIN_CAPTCHA_OPEN);
    ...
    public async Task CheckWebOpen(LoginInput input)
    {
        //判断是否开启web访问
        var webStatus =
            await _configService.GetByConfigKey(CateGoryConst.CONFIG_SYS_BASE, SysConfigConst.SYS_WEB_STATUS);
        ...
        {
            var closePrompt =
                await _configService.GetByConfigKey(CateGoryConst.CONFIG_SYS_BASE, SysConfigConst.SYS_WEB_CLOSE_PROMPT);
    ...
    {
        var lockTime =
            loginPolicy.First(x => x.ConfigKey == SysConfigConst.LOGIN_ERROR_LOCK).ConfigValue.ToInt(); //获取锁定时间
        var errorCount = loginPolicy.First(x => x.ConfigKey == SysConfigConst.LOGIN_ERROR_COUNT).ConfigValue
            .ToInt(); //获取错误次数
    ...
    {
        var resetTime = loginPolicy.First(x => x.ConfigKey == SysConfigConst.LOGIN_ERROR_RESET_TIME).ConfigValue
            .ToInt(); //获取重置时间
        var key = SystemConst.CACHE_LOGIN_ERROR_COUNT; //获取登录错误次数Key值
        //如果是关闭多租户则直接用账号
        if (!isTenant)
    ...
    /// <returns></returns>
    public async Task ValidPhoneValidCode(GetPhoneValidCodeInput input, LoginClientTypeEnum loginClientType,
        bool isDelete = true)
    {
        ...
        if (loginClientType == LoginClientTypeEnum.B) //B端登录
        {
            ...
            if (userId == 0) throw Oops.Bah("手机号不存在"); //手机号不存在
        }
        else
        {
            //校验C段用户手机号
        ...
        if (tokenInfos != null)
        {
            ...
            if (isSingle)
            {
                await SingleLogin(loginEvent.SysUser.Id.ToString(),
                    tokenInfos.Where(it => it.LoginClientType == loginClientType).ToList()); //单用户登录方法
    ...
    private void RemoveTokenFromRedis(LoginEvent loginEvent, LoginClientTypeEnum loginClientType)
    {
        ...
        {
            //获取当前用户的token
            var token = tokenInfos.Where(it => it.Token == loginEvent.Token && it.LoginClientType == loginClientType)
                .FirstOrDefault();
            if (token != null)
    ...
    private List<TokenInfo> GetTokenInfos(long userId)
    {
        //缓存获取用户token列表
        var tokenInfos =
            _simpleCacheService.HashGetOne<List<TokenInfo>>(CacheConst.CACHE_USER_TOKEN, userId.ToString());
        if (tokenInfos != null) tokenInfos = tokenInfos.Where(it => it.TokenTimeout > DateTime.Now).ToList(); //去掉登录超时的
        return tokenInfos;
    }
