...
public class DictService : DbRepository<SysDict>, IDictService
{
    ...
    public async Task<List<SysDict>> Tree(DictTreeInput input)
    {
        ...
        var devList = sysDictList.WhereIF(!string.IsNullOrEmpty(input.Category), it => it.Category == input.Category)
    ...
    private async Task CheckInput(SysDict sysDict)
    {
        ...
        var hasSameLabel = dictList.Any(it =>
            it.ParentId == sysDict.ParentId && it.Category == sysDict.Category && it.DictLabel == sysDict.DictLabel &&
            it.Id != sysDict.Id);
        if (hasSameLabel) throw Oops.Bah($"存在重复的字典名称:{sysDict.DictLabel}");
        //判断是否存在重复字典值
        var hasSameValue = dictList.Any(it =>
            it.ParentId == sysDict.ParentId && it.DictValue == sysDict.DictValue && it.Id != sysDict.Id);
        if (hasSameValue) throw Oops.Bah($"存在重复的字典值:{sysDict.DictValue}");
    }
