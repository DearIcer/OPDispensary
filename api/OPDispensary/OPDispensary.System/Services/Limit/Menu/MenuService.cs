...
public class MenuService : DbRepository<SysResource>, IMenuService
{
    ...
    public async Task<List<SysResource>> ShortcutTree(List<SysResource> sysResources = null)
    {
        ...
        //将单页的排前面根据排序码排序
        tree = tree.OrderByDescending(it => it.Category == CateGoryConst.RESOURCE_SPA).ThenBy(it => it.SortCode)
            .ToList();
        return tree;
    }
    ...
    public async Task Edit(MenuEditInput input)
    {
        ...
        if (updatePath)
        {
            ...
            var userPermissions =
                await _relationService.GetRelationByCategory(CateGoryConst.RELATION_SYS_USER_HAS_PERMISSION);
            //找到所有匹配的权限
    ...
    private async Task<SysResource> CheckInput(SysResource sysResource)
    {
        ...
        //判断是否有同级且同名的菜单
        if (menList.Any(it =>
                it.ParentId == sysResource.ParentId && it.Title == sysResource.Title && it.Id != sysResource.Id))
