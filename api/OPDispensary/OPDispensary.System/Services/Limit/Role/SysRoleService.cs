...
public class SysRoleService : DbRepository<SysRole>, ISysRoleService
{
    ...
    public SysRoleService(ILogger<SysRoleService> logger, ISimpleCacheService simpleCacheService,
    ...
    private async Task CheckInput(SysRole sysRole)
    {
        ...
        var repeatName =
            sysRoles.Any(it => it.OrgId == sysRole.OrgId && it.Name == sysRole.Name && it.Id != sysRole.Id); //是否有重复角色名称
        if (repeatName) //如果有
        {
    ...
    public async Task<List<RoleTreeOutput>> Tree(RoleTreeInput input)
    {
        ...
        foreach (var org in topOrgList)
        {
            ...
            var childRoles =
                sysRoles.Where(it => it.OrgId != null && childIds.Contains(it.OrgId.Value)).ToList(); //获取机构下的所有角色
    ...
    public async Task<List<UserSelectorOutPut>> OwnUser(BaseIdInput input)
    {
        ...
        var userList = await Context.Queryable<SysUser>().Where(it => userIds.Contains(it.Id))
    ...
    public async Task<SqlSugarPagedList<RoleSelectorOutPut>> RoleSelector(RoleSelectorInput input)
    {
        ...
        //如果机构ID列表不为空
        if (input.OrgIds != null) orgIds = orgIds.Where(it => input.OrgIds.Contains(it)).ToList(); //包含在机构ID列表中的组织ID
    ...
    public async Task Edit(RoleEditInput input)
    {
        ...
        if (role != null)
        {
            ...
            if (role.DefaultDataScope.ScopeCategory != input.DefaultDataScope.ScopeCategory)
            {
                //获取角色授权列表
                var relations =
                    await _relationService.GetRelationByCategory(CateGoryConst.RELATION_SYS_ROLE_HAS_PERMISSION);
                //找到当前角色的
                ...
                permissions.ForEach(it =>
                {
                    ...
                    if (rolePermission.ScopeCategory == role.DefaultDataScope.ScopeCategory
                        && rolePermission.ScopeDefineOrgIdList.SequenceEqual(role.DefaultDataScope
    ...
    public async Task GrantResource(GrantResourceInput input)
    {
        ...
        if (sysRole != null)
        {
            ...
            var result = await Tenant.UseTranAsync(async () =>
            {
                ...
                //如果不是代码生成,就删除老的
                if (!input.IsCodeGen)
                    await relationRep.DeleteAsync(it => it.ObjectId == sysRole.Id &&
    ...
    public async Task GrantPermission(GrantPermissionInput input)
    {
        ...
        if (sysRole != null)
        {
            ...
            var extJsons = input.GrantInfoList.Select(it => it.ToJson()).ToList(); //拓展信息
            await _relationService.SaveRelationBatch(CateGoryConst.RELATION_SYS_ROLE_HAS_PERMISSION, input.Id, apiUrls,
                extJsons,
                true); //添加到数据库
            await _eventPublisher.PublishAsync(EventSubscriberConst.CLEAR_USER_CACHE,
                new List<long> { input.Id }); //清除角色下用户缓存
    ...
    public async Task GrantUser(GrantUserInput input)
    {
        ...
        var result = await Tenant.UseTranAsync(async () =>
        {
            ...
            await relationRep.DeleteAsync(it =>
                it.TargetId == targetId && it.Category == CateGoryConst.RELATION_SYS_USER_HAS_ROLE); //删除老的
    ...
    public async Task Delete(BaseIdListInput input)
    {
        ...
        if (ids.Count > 0)
        {
            var sysRoles = await GetListAsync(); //获取所有角色
            var hasSuperAdmin =
                sysRoles.Any(it => it.Code == SysRoleConst.SUPER_ADMIN && ids.Contains(it.Id)); //判断是否有超级管理员
            ...
            var result = await Tenant.UseTranAsync(async () =>
            {
                ...
                //删除关系表角色与用户关系
                await relationRep.DeleteAsync(it =>
                    targetIds.Contains(it.TargetId) && it.Category == CateGoryConst.RELATION_SYS_USER_HAS_ROLE);
            });
            if (result.IsSuccess) //如果成功了
            {
                await RefreshCache(); //刷新缓存
                ...
                await _relationService.RefreshCache(CateGoryConst
                    .RELATION_SYS_ROLE_HAS_MODULE); //关系表刷新RELATION_SYS_ROLE_HAS_MODULE缓存
