...
namespace OPDispensary.Web.Core;
...
public class DatabaseLoggingWriter : IDatabaseLoggingWriter
{
    ...
    private async Task CreateVisitLog(string operation, LoggingMonitorJson loggingMonitor, ClientInfo clientInfo)
    {
        ...
        if (operation == EventSubscriberConst.LOGIN_B)
        {
            ...
            var userInfo = result.ToJsonEntity<OPDispensaryResult<SysUser>>(); //格式化成user表
        ...
        else
        {
            ...
            name = loggingMonitor.AuthorizationClaims.Where(it => it.Type == ClaimConst.NAME).Select(it => it.Value)
                .FirstOrDefault();
            opAccount = loggingMonitor.AuthorizationClaims.Where(it => it.Type == ClaimConst.ACCOUNT)
    ...
    {
        //用户名称
        var name = loggingMonitor.AuthorizationClaims?.Where(it => it.Type == ClaimConst.NAME).Select(it => it.Value)
            .FirstOrDefault();
        //账号
        var opAccount = loggingMonitor.AuthorizationClaims?.Where(it => it.Type == ClaimConst.ACCOUNT)
            .Select(it => it.Value).FirstOrDefault();
        ...
        await _db.CopyNew().InsertableWithAttr(devLogOperate).IgnoreColumns(true).SplitTable()
            .ExecuteCommandAsync(); //入库
    }
